...
//					}
			...
			while (pop2.size() < popSize) {
				...
				if (random.nextDouble() < mutationRate) {
					if (random.nextBoolean()) {
						//crashes here, there must be something different about children generated by OX
	...
	private ArrayList<Individual> orderCrossover(Individual parent1, Individual parent2) {
		...
		for (int i = 0; i < parent1.chromosome.size(); i++) {
			oxchild1.chromosome.add(null);
			oxchild2.chromosome.add(null);
		}
		...
		for (int i = 0; i <= size; i++){
			...
			if(!oxchild1.contains(currentLocationInParent2)){
				oxchild1.chromosome.add(currentLocationInParent2);
			}

			if(!oxchild2.contains(currentLocationInParent1)){
